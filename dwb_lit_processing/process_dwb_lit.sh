...
## COMPUTE DWB AND LIT
r.mapcalc --overwrite expression=\"dw_bionamss_100m = if(isnull(${AGB_RCL}), null(), ${AGB} * dwb_coeffs )\" &					              
...
wait
grass ${CARBON_MAPSET_PATH} --exec r.support map=dw_bionamss_100m title="Dead Wood Biomass map" units="Mg/ha" description="Derived from AGB map" >>${LOGPATH}/compute_dwb_lit.log 2>&1
