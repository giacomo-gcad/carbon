...
# # CEP TILES WITH ONLY ONE CID ARE ANALYZED AT IN_RASTER RESOLUTION
# ## commenting/uncommenting 'align=${IN_RASTER}' allows to perform the analysis, respectively, at cep/${IN_RASTER} resolution
...
" > ./dyn/runivar_${eid}_${qid}.sh
	...
	# POSTPROCESS RESULTS
	# remove headers
	sed -i.bak '/zone|label|non_null_cells|null_cells|min|max|range|mean|mean_of_abs|stddev|variance|coeff_var|sum|sum_abs/d' ${SUBDIR}/z_${OUTCSV}_${qid}.csv
	# replace '-nan' with 0
